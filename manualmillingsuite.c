...
void print_xy_position(double x, double y, double dt, int linecount) {
  ...
  if (x < 0.0 || y < 0.0) {
    ...
    printf("\n\n#%d Position\tX = %3.4fin %3.2fmm   Y = %3.4fin %3.2fmm   %s",
           linecount, x, x / MM, y, y / MM, wp);
    free_malloc(wp);
  }
}
...
void print_layout(void) {
  ...
  printf("\n\n Fractions are allowed in numeric fields");
...
int get_english_or_metric(void) {
  ...
  choice = choose_one("\n\nEnter <I>nches or <M>illimeters: ", "im");
...
int get_inside_outside(void) {
  ...
  int choice = choose_one("\n\nEnter <I>nside or <O>utside Radius: ", "io");
...
void xy_distance_angle(double dt, int metric_flag) {
  ...
  printf("\n\nMidpoint: \tX = %3.4fin %3.2fmm   Y = %3.4fin %3.2fmm   %s", mx,
         mx / MM, my, my / MM, wp);
  free_malloc(wp);
}
...
void abs_xy_wheel(double dt, int metric_flag) {
  ...
  printf("\n\nWheel Position:\tX = %3.4fin %3.2fmm   Y = %3.4fin %3.2fmm   %s",
         sx, sx / MM, sy, sy / MM, wp);
  free_malloc(wp);
}
...
void rel_xy_wheel(double dt, int metric_flag) {
  ...
  wp = wheel_position(sx, sy, dt);
  printf("\n\nStarting at \tX = %3.4fin %3.2fmm   Y = %3.4fin %3.2fmm   %s", sx,
         sx / MM, sy, sy / MM, wp);
  free_malloc(wp);
  ...
  while (TRUE) {
    ...
    printf("\n\nEnding at \tX = %3.4fin %3.2fmm   Y = %3.4fin %3.2fmm   %s", dx,
           dx / MM, dy, dy / MM, wp);
    free_malloc(wp);
...
void tap_drill_size(int metric_flag) {
  ...
  if (metric_flag) {
    ...
    hd =
        fabs(get_fraction("\nEnter Bolt Head Diameter (0 to Skip "
                          "Clearance Hole Calculation): "));
...
void ellipse(double dt, int metric_flag) {
  ...
  do {
    ...
    foci = major_radius -
           sqrt(major_radius * major_radius - minor_radius * minor_radius);
    if (c == 0)
  ...
  r_radians = deg_to_rad(360.0 - fmod(rotation_angle, 360.0));
  ...
  for (a = startangle; a <= endangle; a += 0.001) {
    ...
    x0 = centerxoffset + (major_radius * cos(radians) * cos(r_radians) -
                          minor_radius * sin(radians) * sin(r_radians));
    y0 = centeryoffset - (major_radius * cos(radians) * sin(r_radians) +
                          minor_radius * sin(radians) * cos(r_radians));
...
void material_removal(int metric_flag) {
  ...
  if (metric_flag) {
    ...
    printf("\n\nMaterial Removal in Cubic Millimeters per Minute:\t%3.1f",
           ipm * woc * doc);
  } else {
    ipm = fabs(get_fraction("\nEnter Feed Rate in Inches per Minute: "));
    woc = fabs(get_fraction("\nEnter Width of Cut: "));
    ...
    printf("\n\nMaterial Removal in Cubic Inches per Minute:\t%3.3f",
           ipm * woc * doc);
  }
}
...
int main(void) {
  ...
  printf("\n\n\n");
  printf("\t\t\tManual Milling Suite v2.0");
  printf("\n\n");
